 ...
 * simulates a quad traveling north, west, east, west and returning to home
 *       
...
testServoRangeError(); //moves horz servo through 90,0,90,180,90,0,90 degrees, vert servo through 90,180,90 degrees
                       // adjust mapping in Declare Constants above to correct for servo error.


decode.begin();//start telemetry decoder

//get tracker location details
doTest(); //simulates the tracking of a quad
...
void servosToHome()
  ...
  void servoHorzMove(int from, int to, int servoDelay)
    ...
    if(from < SERVO_HORZ_MIN)
      ...
      from = ROTATE - from;//correct for anticlock rotation
      to = ROTATE - to;
      if(from < to)
        {
...
//***********servo vert move***********
    ...
    void testServos()
      {
        ...
        servoHorzMove(SERVO_HORZ_MIN, SERVO_HORZ_HOME, SERVO_SLOW_DELAY );
        servoHorzMove(SERVO_HORZ_HOME, SERVO_HORZ_MAX, SERVO_SLOW_DELAY );
  ...
  void doTest()
  {
   ...
   debugln(F("***Going North***"));
    ...
    debugln(F("***Going West***"));
    for(int n = 0; n <= 100; n++)//fly west and gain height
    {
      //decode.decodedLat = decode.decodedLat - 0.00020;
      decode.decodedLon = decode.decodedLon - 0.00020;
      decode.decodedAltitude = decode.decodedAltitude + 20;
    ...
    debugln(F("***Going East***"));
    for(int n = 0; n <= 200; n++)//fly east at loose height
    {
      //decode.decodedLat = decode.decodedLat - 0.00020;
      decode.decodedLon = decode.decodedLon + 0.00020;
      decode.decodedAltitude = decode.decodedAltitude - 15;
      decode.getQuadLocation();
      driveTracker();
    }
    debugln(F("***Going West***"));
    ...
    debugln(F("***Going South***"));
    for(int n = 0; n <= 45; n++)//come home loosing height
    {
      decode.decodedLat = decode.decodedLat - 0.00020;
      //decode.decodedLon = decode.decodedLon + 0.00020;
      decode.decodedAltitude = decode.decodedAltitude - 40;
      decode.getQuadLocation();
      driveTracker();
    }
    delay(1000);
